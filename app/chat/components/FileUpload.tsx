...
function getFileColor(mimeType?: string): string {
  if (!mimeType) return 'from-[#FF8B3D] to-[#FF8B3D]';
      ...
      {attachments.length === 0 && (
        <div
          ...
          className={`relative border-2 border-dashed rounded-2xl p-8 transition-all duration-300 ${
            dragActive
              ? 'border-[#FF8B3D] bg-gradient-to-br from-[#FF8B3D]/10 to-[#d9753e]/5 scale-[1.02]'
              : 'border-gray-300 hover:border-[#FF8B3D]/50 hover:bg-gray-50/50'
          }`}
        >
          {dragActive && (
            <div className="absolute inset-0 bg-[#FF8B3D]/5 rounded-2xl animate-pulse pointer-events-none"></div>
          )}
            ...
            <div className={`relative mb-4 ${uploading ? 'animate-bounce' : ''}`}>
              {uploading ? (
                <div className="relative">
                  <div className="absolute inset-0 bg-[#FF8B3D]/20 rounded-full blur-xl"></div>
                  <Loader2 className="relative w-12 h-12 text-[#FF8B3D] animate-spin" />
                </div>
              ) : (
                <div className="relative group">
                  <div className="absolute inset-0 bg-gradient-to-br from-[#FF8B3D]/20 to-[#d9753e]/20 rounded-full blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                  <div className="relative bg-gradient-to-br from-[#FF8B3D] to-[#d9753e] p-3 rounded-2xl shadow-lg group-hover:shadow-xl transition-all duration-300 group-hover:scale-110">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <label
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-[#FF8B3D] bg-[#FF8B3D]/10 hover:bg-[#FF8B3D]/20 rounded-lg cursor-pointer transition-colors"
            >
              {uploading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {attachments.map((attachment, index) => (
              <div
                key={attachment.id}
                className="group relative border border-gray-200 rounded-xl p-3 hover:border-[#FF8B3D] hover:shadow-md transition-all duration-200 bg-white"
                style={{
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {attachment.fileType === 'document' && attachment.extractedText && (
                        <>
                          <span className="text-xs text-gray-400">•</span>
                          <Check className="w-3 h-3 text-[#FF8B3D]" />
                        </>
                      )}
                    </div>
                  </div>
