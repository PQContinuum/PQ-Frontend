        ...
        {options.map((option) => {
          ...
          return (
            ...
            >
              {/* Selection indicator */}
              {isSelected && (
                <div className="absolute top-3 right-3 w-4 h-4 bg-[#FF8B3D] rounded-full flex items-center justify-center">
                  <Check className="w-2.5 h-2.5 text-white" />
                </div>
              )}

              {/* Icon */}
              <div className={`p-2.5 rounded-xl shrink-0 ${isSelected ? 'bg-[#FF8B3D]/10' : 'bg-gray-100'}`}>
                {IconComponent && (
                  <IconComponent className={`w-6 h-6 ${isSelected ? 'text-[#FF8B3D]' : 'text-gray-600'}`} />
                )}
              </div>
