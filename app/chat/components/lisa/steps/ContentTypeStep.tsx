          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center gap-4 p-6 rounded-2xl border-2 transition-all duration-200 ${
                isSelected
                  ? 'border-[#00552b] bg-[#00552b]/5 shadow-lg shadow-[#00552b]/10'
                  : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
              }`}
            >
              {/* Selection indicator */}
              {isSelected && (
                <div className="absolute top-3 right-3 w-5 h-5 bg-[#00552b] rounded-full flex items-center justify-center">
                  <Check className="w-3 h-3 text-white" />
                </div>
              )}

              {/* Icon */}
              <div className={`p-4 rounded-2xl ${isSelected ? 'bg-[#00552b]/10' : 'bg-gray-100'}`}>
                {IconComponent && (
                  <IconComponent className={`w-10 h-10 ${isSelected ? 'text-[#00552b]' : 'text-gray-600'}`} />
                )}
              </div>
