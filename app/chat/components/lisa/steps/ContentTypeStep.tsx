          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center gap-4 p-6 rounded-2xl border-2 transition-all duration-200 ${
                isSelected
                  ? 'border-[#FF8B3D] bg-[#FF8B3D]/5 shadow-lg shadow-[#FF8B3D]/10'
                  : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
              }`}
            >
              {/* Selection indicator */}
              {isSelected && (
                <div className="absolute top-3 right-3 w-5 h-5 bg-[#FF8B3D] rounded-full flex items-center justify-center">
                  <Check className="w-3 h-3 text-white" />
                </div>
              )}

              {/* Icon */}
              <div className={`p-4 rounded-2xl ${isSelected ? 'bg-[#FF8B3D]/10' : 'bg-gray-100'}`}>
                {IconComponent && (
                  <IconComponent className={`w-10 h-10 ${isSelected ? 'text-[#FF8B3D]' : 'text-gray-600'}`} />
                )}
              </div>
              ...
              <div className="text-center">
                <h3 className={`text-lg font-semibold ${
                  isSelected ? 'text-[#FF8B3D]' : 'text-gray-900'
                }`}>
                  {option.label}
                </h3>
                <p className="text-sm text-gray-500 mt-1">
