...
export function VideoImageUpload({
  ...
  if (previewUrl && !isUploading) {
    return (
      <div className="relative group">
        <div className="relative w-12 h-12 rounded-lg overflow-hidden border-2 border-[#934f2c]/60 shadow-sm">
          <img
            src={previewUrl}
  ...
  return (
    ...
    >
      ...
      {isUploading ? (
        <Loader2 className="size-5 text-[#934f2c] animate-spin" />
      ) : error ? (
        <div className="absolute inset-0 flex items-center justify-center p-1">
          <span className="text-[8px] text-red-500 text-center leading-tight">{error}</span>
        </div>
      ) : (
        <ImagePlus className={`size-5 ${isDragging ? 'text-[#934f2c]' : 'text-gray-400'}`} />
      )}
    </div>
  );
}
